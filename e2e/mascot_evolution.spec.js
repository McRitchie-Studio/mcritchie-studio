...
// Croconaw submitted it, Feraligatr assembled it — and each historical card
...
test("task timeline shows the mascot evolving through the gates", async ({ page }) => {
  ...
  expect(await crew("Feraligatr").count()).toBeGreaterThanOrEqual(1); // the assemble gate's form
});
