...
| **`deploy-with-task`** (Avi act; ship authority for ONE task) | [`agents/avi/sops/deploy-with-task.md`](../agents/avi/sops/deploy-with-task.md): **GUARD `release == main`** → `review-one <task>` (→ `reviewed`) → `qa-release` (sweeps + merges it) → `production-deploy`. Interactive — launched bare it asks "What task?". *Formerly the `Deploy with Task <task>` write-up here.* |
...
### 3.3 Definition of Ready for review (DoR) — the enforcement (the G1 Cert gate)
...
The whole §3.3 sequence is recorded as the task's **G1 Cert** gate
(attempt-aware `GateRun` rows): the cert tools open the `g1_cert` attempt and
append one SOP per lane, a red lane closes it `failed` (the re-run opens
attempt n+1), and the merge-gate dor-check verdict closes a green run. The
reviewer's gate-zero re-run uses `bin/dor-check <task> --gate-role review` so
its verdict lands on the **G2a** review lane instead of closing the builder's
gate. Standalone gate SOPs: `docs/agents/modules/gates/g1-cert.md` /
