...
> agent's Turf Monster admin identity and local `SOLANA_ADMIN_KEY`; it moved no
> on-chain membership. `8K81…` is still the Squads signer and still production's
> `SOLANA_ADMIN_KEY`, so this worked example rotates the key it always did.
> Mr. McRitchie decided that day to retire `8K81…` from the signer set — when
> that is actually transacted, this example is the procedure that does it.
