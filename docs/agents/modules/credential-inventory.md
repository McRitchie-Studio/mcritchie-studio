...
| `higgsfield.studio.agents` | `studio-agents` | Higgsfield media generation API (`api.higgsfield.ai`). Filed 2026-09-20 under the `<service>.<entity>.<lane>` convention, category `API Credential`. Fields **`API Key ID`** → `HIGGSFIELD_API_KEY` and **`API Key Secret`** → `HIGGSFIELD_API_SECRET`. ⚠ **The auth shape is an OPEN QUESTION.** `app/services/higgsfield/client.rb` sends both halves in ONE header, `Authorization: Key <id>:<secret>`, verified against the live API 2026-09-20 — but the dashboard key dialog now appears to show a SINGLE key. If Higgsfield issues one bare key, this item collapses to one field and the client header is a code change; confirm against the dashboard before rotating. Supersedes `agent.higgesfield`. | McRitchie Studio content pipeline (`Content::AssetsAgent`, `Content::AssembleAgent`); local `.env` only — `mcritchie-studio` carried no `HIGGSFIELD_*` config on 2026-09-20 |
