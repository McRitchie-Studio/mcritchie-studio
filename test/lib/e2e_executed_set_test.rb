...
class E2eExecutedSetTest < Minitest::Test
  ...
  # BLOCKER B. A widened `--grep-invert '@quarantine|board'` does not SKIP the specs it
  ...
  # The quarantined specs are RED — that is why they are excluded. If one EXECUTES, the CI
  # command and the contract have drifted apart, and the count alone might still add up.
  def test_unit_an_executed_quarantined_spec_is_red
    reports = [
      report(source: "shard-1.json", shard: 1, total: 2,
             specs: [["landing page loads", "expected"], ["activities page loads @quarantine", "expected"]]),
      report(source: "shard-2.json", shard: 2, total: 2, specs: [["agents page loads", "expected"]])
    ]

    failures = gate(reports).failures

    assert_match(/carry @quarantine in the title/, failures.join)
  end
