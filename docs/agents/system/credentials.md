...
1. **Restore a working upgrade path.** `squad-upgrade.js` signs as `8K81…` and cosigns with `CytJ…`. `CytJ…` is off both Squads; `8K81…` is off mainnet's, and devnet transaction #18 re-seated it on devnet's. So the script can approve nothing on mainnet, lands only its `8K81…` approval on devnet, and cannot drive an upgrade on either cluster today. A current member must sign — the agent holds `BLSBw8…` — with three of the five approving.
