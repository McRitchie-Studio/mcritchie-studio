...
| `agent.solana` | `studio-agents` | Legacy Xan Solana wallet (the identity was called "Alex Bot" until 2026-09-15); retired after key rotation, and superseded by `agent.xan.solana` below. Carries no `private key` field (verified 2026-09-14), so it cannot sign anything. | Historical reference only |
| `agent.xan.solana` | `studio-agents-admin` | The **Xan** signer (`8K81…`) — the rotated replacement for `agent.solana` above, renamed from `agent.alex.solana` and MOVED out of `studio-agents` on 2026-09-15. NOT the human Alex Phantom wallet (`7ZDJ…`), which has no item in any vault. **An ordinary agent service account cannot see this vault, and that is the control** — it is what leaves an agent at 1-of-3 rather than 2-of-3 on both Squads multisigs. Read it under `source ~/.zprofile.admin`; the field label is SPACED (`private key`), verified 2026-09-15. Consumers that still name the OLD identity are turf-vault code, not stale prose: `squad.json` → `members.alex_bot` and `squad-upgrade.js` → `ALEX_BOT_KEY`. | turf-vault and Turf Monster ops |
...
| `agent.turf.solana` | `studio-agents` | The Turf Monster **house wallets** — **not** a Squads member and **not** a `VaultState.signers` entry (verified 2026-09-14). Now holds BOTH system wallets: mainnet `wallet-address` **`BLSBw8fXHzZc5pbaYCKMpMSsrtXBTbWXpUPVzMrXx9oo`** and devnet `devnet-wallet-address` **`2eGs8G3wzhEeNQQU2Q86BmmA2xTpDbMMae3Y1bvpZfx9`**, with HYPHENATED field labels where the older agent.* items use spaces. **⚠ TWO ITEMS SHARE THIS EXACT TITLE** (verified still true 2026-09-15): `mczgzinhh42mlltd6h4yvladhi` (newer — both wallets, hyphenated) and `wriypyvp7rn2q2snqizhyaojku` (older — mainnet only, spaced). A title read matches both and `op` refuses with "More than one item matches", so address it by ID; `turf_monster/qa_rehearsal/key_store.rb` pins the newer one. This row previously withheld the address as "not published elsewhere" — that was false: `BLSBw8fX…` is in three tracked files of the PUBLIC turf-monster repo, and a wallet address is public by construction anyway. | Turf Monster house flows |
