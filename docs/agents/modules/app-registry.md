...
| Cyvasse | planned managed satellite | 3600 | 3600-3699 | Hex strategy board game, revived from Alex's first app (2014-15). Tier decision (2026-09-25, epic `cyvasse-revival`): **managed satellite** on studio-engine, three-rung ladder from birth, registered in `config/satellites.yml` (planned) and `config/release_repos.yml` (three-rung). No `prod_deploy`, gate commands or QA environment yet: the Heroku app `cyvasse` and the `cyvasse.mcritchie.studio` host arrive with the hosting piece, and until then a sweep holds a cyvasse member at the QA stamp. Repo `McRitchie-Studio/cyvasse` (public) |

Do not reuse `3200-3699`. Rolio is already in `config/satellites.yml` with
