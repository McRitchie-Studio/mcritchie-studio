...
- **Heroku apps**: `mcritchie-studio` → https://mcritchie.studio; `turf-monster-mainnet` → https://app.turfmonster.media. `app.mcritchie.studio` remains a legacy Rails alias, `v1.mcritchie.studio` is the previous Squarespace site, and QA runs at `qa.mcritchie.studio` / `qa.turfmonster.media`. `RAILS_MASTER_KEY` shared across apps via Heroku config.
