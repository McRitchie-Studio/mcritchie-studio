...
- **`full-cycle`** (`Xan Heartbeat` act; full ship authority) — the whole
...
|---|---|
| **`full-cycle`** (Xan Heartbeat act; full ship authority) | [`agents/xan/sops/full-cycle.md`](../agents/xan/sops/full-cycle.md): `pr-review` → `qa-release` → `production-deploy` — the whole release, review to prod. *Formerly the retired `Merge, Assemble, Deploy` chip;* named `full-cycle` to avoid colliding with the read-only `bin/devops-cycle` snapshot tool. |
...
`Xan Heartbeat` / `Turf Monster Heartbeat`) plus one or more **copyable action
...
| **Xan** (`Xan Heartbeat`) | `share-insights` · `full-cycle` · `grade-events` (optional since 2026-09-25: every task is graded once at ship) | grade the 10 most recent resolved activities at `/xan/heartbeat` only when named; share the banked insights out (regenerate the lessons doc from `ActionGrade.banked`; it installs nothing); OR run the whole cycle review→assemble→QA→prod ship (`full-cycle`, full ship authority) | 10 graded + insights banked; the bank shared out; or the whole release `shipped` |
...
2. **Carl picks his domain LIGHT.** Carl runs **`bin/reviewer-select <task>
   --no-record`** — it
   ...
   **domain fit** (the task's shape + repositories + risk tags vs each soul's
   ...
   build move from `--actor <soul>`, else `devops.persona`, else the assigned
   `agent_slug` — so a soul never reviews their own work with **no manual
   flag**), **and any busy souls** you name. **`--no-record` is what makes that a
   PREVIEW.** A bare run RECORDS the pair, and recording first ACQUIRES the task's
   review claim — a ~3h25m lease (`ClaimLease::REVIEW_TTL_SECONDS`) with no renewer
   behind it, which drops the task out of `Task.reviewable` and out of `bin/task
   ...
   review kicks off, before `→reviewed` lands. `--record` remains only as a
   back-compat no-op; `--no-record`/`--dry` is the opt-OUT.)
