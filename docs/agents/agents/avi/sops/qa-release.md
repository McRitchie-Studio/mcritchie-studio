...
- **ABORT** — `prepare` reached a verdict and refused: a red pre-QA gate, a failed
  QA boot, a never-dispatched QA deploy, a failed merge. **Fix the cause first,
  THEN re-run.**
...
| **QA DEPLOY NEVER DISPATCHED** (step 4c — "`<workflow>` was dispatched but GitHub registered NO run for it — the deploy NEVER RAN") | **Do not touch the app — it was never redeployed, and it is not at fault.** `gh workflow run` can accept a dispatch, exit 0, print nothing, and leave GitHub with no run at all (measured on rel-20260907-14cff2). The abort names the workflow, the SHA, and the exact dispatch command; run that command by hand and confirm a run registers (`gh run list --workflow <workflow> --limit 3`). **Do NOT reach for `bin/qa-server deploy`, and do NOT lengthen the boot poll** — before this abort existed, this failure surfaced as the row below ("never returned /up 200") and sent an hour into a healthy app. Nothing was deployed, promoted or recorded | re-run `prepare` once a manual dispatch registers a run |
| **QA deploy / boot FAILED** (the app WAS deployed to and did not come up) | Fix the boot failure (the summary prints the `bin/qa-server deploy …` retry); eject the member if it is the cause. **This row is now only reachable by an app that was actually deployed to** — a dispatch that created no run aborts at the row above instead of arriving here as a boot timeout | re-run `prepare` **once QA boots** |
