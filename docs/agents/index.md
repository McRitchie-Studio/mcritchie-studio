...
4. If the work is a feature, bug, QA, release, cleanup, or active-doc change,
   ...
   implementation. Record acceptance criteria, affected repos, risk tags,
   expected checks in `devops["test_plan"]`, and
   `devops["worktree_slug"]`. The production task slug is immutable and
   generated by the app; use `devops["worktree_slug"]` for the human-readable
   feature handle. Move the task to `in_progress` once the agent starts work.
...
9. Commit and push the feature branch when the work is coherent, then run
   ...
   Update the task with branch, PR URL, local URL,
   `devops["checks_run"]`, and any changed acceptance criteria, then move it
   to `pr_review`. Handoffs should include the task URL before the PR URL.
...
criteria, affected repos, risk tags, expected checks in devops["test_plan"], and
devops["worktree_slug"]. Use an isolated worktree and allocated port before
editing. Bind the production task URL to the worktree. Give me a local URL to
review, update docs if behavior changes, then commit, push the feature branch,
run bin/agent-worktree finish, update the task with branch/PR/local URL and
devops["checks_run"], and move it to pr_review for Avi QA. Lead the handoff
with the task URL. Do not merge or deploy unless I explicitly assigned that
lane.
```
