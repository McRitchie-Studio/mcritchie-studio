class Release
  ...
  module Conductor
    ...
    # verdict was unverified), "credited" (the credited source when a fast-forwarded
    # promote let the gate credit the SHA's existing green conclusion instead of
    # awaiting a duplicate run — bin/release's ci_credit_verdict) — each present
    # only when the gate had it. Recorded so a DISAGREEMENT is auditable after the
