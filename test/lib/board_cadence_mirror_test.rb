...
#   JS    — BEAT_MS in tasks/_deployments_live_fx, milliseconds. Every exit animation
#           is timed to finish inside one beat; a beat that shrank without them would
#           put two cards on screen at once, and one that grew would leave dead air.
#
...
class BoardCadenceMirrorTest < ActiveSupport::TestCase
  ...
  test "[unit] the board's beat is the cadence, in milliseconds" do
    assert_equal (Release::BOARD_FLIP_CADENCE * 1000).round, fx_constant("BEAT_MS"),
                 "BEAT_MS is the same beat the server schedules, expressed in ms"
  end

  test "[unit] every exit animation finishes inside one beat" do
    beat = fx_constant("BEAT_MS")
    %w[EXIT_MS GAP_CLOSE_MS].each do |name|
