...
# Canonical type → display color, in National-Dex type order. Position is the
# array index so a UI can list them in this familiar order.
POKEMON_TYPE_COLORS = [
...
type_position = POKEMON_TYPE_COLORS.each_with_index.to_h { |(key, _l, _c), i| [key, i] }
counts = Hash.new(0)
...
POKEMON_TYPE_COLORS.each do |(key, label, color)|
