...
- **A root `CLAUDE.md` adapter IS required** and is generated alongside
  `AGENTS.md` by `bin/install-agent-docs`. Claude Code auto-loads `CLAUDE.md`
  (and its `@imports`) but does **not** auto-load `AGENTS.md` — proven 2026-06-20
  when multiple Claude sessions skipped the DevOps cycle entirely because
  `AGENTS.md` was never in their context. Source: `docs/agents/claude.md`.
...
adapter (inline routing gate + `@AGENTS.md`), generated by `bin/install-agent-docs`.
