...
| `SOLANA_DEVNET_RPC_URL` (Heroku config var, `mcritchie-studio` production) | env var | Orphaned by [`retire-signing-console`](https://mcritchie.studio/tasks/retire-signing-console): zero hub code reads it once that work ships to production, and the PR deleted the `.env.example` block that was its only in-repo pointer. Its value carries a live Helius API key, so it is a standing credential on an app with no consumer for it. **Measured 2026-09-04:** set on prod `mcritchie-studio`; `SOLANA_MAINNET_RPC_URL` is not set there; `mcritchie-studio-qa` has neither. **The key is SHARED, not the hub's own** — its credential is byte-identical to the one in `turf-monster-mainnet`'s live `SOLANA_RPC_URL` (same Helius key, different cluster host: `devnet.helius-rpc.com` vs `mainnet.helius-rpc.com`; compared by digest, values never printed). | Unset it **only after the production ship** of `retire-signing-console` — running prod still serves the console until then — with `heroku config:unset SOLANA_DEVNET_RPC_URL -a mcritchie-studio`. Do **not** route it through `post_deploy_cmd`: that also fires on the QA deploy and would unset the prod var early. Unsetting the hub's copy is safe on its own; **rotating** the Helius key is not, because it would break turf-monster mainnet RPC — rotate only alongside `turf-monster-mainnet`. | pending production ship |
