...
- **Pre-QA suite** (`pre_qa_gate` SOPs, one per app) — each app's registered
  ...
  **~18x slower** under concurrent-agent load (4.6s idle → 85s loaded). Rolio's
  gate is therefore safe **only with** the wait-budget fix that sizes its suite
  for the slow host (rolio task `size-rolio-system-wait-budget`); **the two must
  land together.** Widening a gate onto a suite you have not driven **here** is how
