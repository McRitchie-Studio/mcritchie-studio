...
| `agent.solana` | `studio-agents` | Legacy Alex Bot Solana wallet; retired after key rotation, and superseded by `agent.alex.solana` below. Carries no `private key` field (verified 2026-09-14), so it cannot sign anything. | Historical reference only |
| `agent.alex.solana` | `studio-agents` | The **Alex Bot** signer (`8K81…`) — the rotated replacement for `agent.solana` above. NOT the human Alex Phantom wallet (`7ZDJ…`), which has no item in any vault; "Alex" in this slug names the bot that acts for him. `8K81…` is what `turf-vault/scripts/squad.json` and `docs/CURRENT_DEPLOYMENT.md` both call "Alex Bot signer". | turf-vault and Turf Monster ops; `squad-upgrade.js` reads it as `ALEX_BOT_KEY` |
...
| `agent.turf.solana` | `studio-agents` | The Turf Monster **house wallet** — **not** a Squads member and **not** a `VaultState.signers` entry (verified 2026-09-14). Address deliberately not recorded here: this repo is public and the address is not published elsewhere. | Turf Monster house flows |
