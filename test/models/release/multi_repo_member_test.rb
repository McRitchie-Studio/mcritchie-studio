...
class Release::MultiRepoMemberTest < ActiveSupport::TestCase
  ...
  # The incident's exact task shape: two repos, PR urls per repo (so it clears the
  # sweep's coverage refusal), `reviewed` and ready to ride.
  def two_repo_task(label = "security", pr_urls: { HUB => HUB_PR, TURF => TURF_PR })
    Task.create!(
  ...
  # --- the sweep-time refusal (the interim safety net, kept as the backstop) ---

  test "[unit] validate_members! REFUSES a multi-repo member with an incomplete PR record" do
    task = two_repo_task("incomplete", pr_urls: {}) # only the hub's singular pr_url
    release = Release::Conductor.sweep!(task)
    ...
    assert_match(/turf-monster/, error.message)
    assert_match(/--pr-url-for/, error.message, "the refusal must name the command that clears it")
  end
