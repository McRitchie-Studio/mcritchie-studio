...
- **About to write a migration** → Self-flag with `bin/task update <task-slug> --requires-migration` before touching `db/migrate/`, even if Avi forgot to flag the ticket. There is no lane to take ([`exclusive-lanes.md`](../../system/exclusive-lanes.md)); the duplicate-migration collision check in dor-check and ship is the protection.
- **Multiple concurrent migration tasks in the backlog** → Flag to Avi to batch.
...
- **Migration advice** — I advise on `--requires-migration` flagging and on batching schema changes (per [`exclusive-lanes.md`](../../system/exclusive-lanes.md))
...
- [`exclusive-lanes.md`](../../system/exclusive-lanes.md) — migrations take no lane; I self-flag `--requires-migration` before writing one
