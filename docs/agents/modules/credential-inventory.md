...
| `higgsfield.studio.agents` | `studio-agents` | Higgsfield media generation API (`api.higgsfield.ai`). Filed 2026-09-20 under the `<service>.<entity>.<lane>` convention, category `API Credential`. Fields **`API Key ID`** → `HIGGSFIELD_API_KEY` and **`API Key Secret`** → `HIGGSFIELD_API_SECRET`. **A credential is TWO values** — settled 2026-09-20 against Higgsfield's docs: a key ID plus a secret, sent in one header as `Authorization: Key <id>:<secret>`, which is what `app/services/higgsfield/client.rb` already sends. ⚠ The console re-displays the **key ID** but shows the **secret only once at creation**, so a console view of one masked value is half the pair, not a single-key credential; rotate both fields together or it authenticates as neither. Supersedes `agent.higgesfield`. | McRitchie Studio content pipeline (`Content::AssetsAgent`, `Content::AssembleAgent`); local `.env` only — `mcritchie-studio` carried no `HIGGSFIELD_*` config on 2026-09-20 |
| `agent.higgesfield (RETIRED - use higgsfield.studio.agents)` | `studio-agents` | **Retired 2026-09-20**, superseded by the row above. It was both the legacy inverted `agent.<service>` form and a misspelling; the `credential-filing` SOP grandfathers legacy names but says to rename one when you next touch it and fix every reference in the same pass — which is what that date was. Its value is kept as a fallback until the replacement key is proven to authenticate, then archive it. **Do not rotate it** — no script, `.env` template or doc points here any more. | Nothing — pending archive |
