...
| `agent.higgesfield (RETIRED - use higgsfield.studio.agents)` | `studio-agents` | **Retired 2026-09-20**, superseded by the row above. It was both the legacy inverted `agent.<service>` form and a misspelling; the `credential-filing` SOP grandfathers legacy names but says to rename one when you next touch it and fix every reference in the same pass — which is what that date was. Its value is kept as a fallback until the replacement key is proven to authenticate, then archive it. **Do not rotate it.** No `op://` lookup resolves it — swept across every repo on disk 2026-09-20, zero hits. But the census is not quite complete: `.env.example:48` still names this item (and the wrong vault, `agents`). A comment-only edit there trips the docs shape gate, which classifies by FILE not hunk, so it was deliberately left behind and tracked as `point-env-example-at-higgsfield`. **Archive this item only after that lands** — `.env.example` is the fresh-machine template, so archiving first sends a rebuild to a name that no longer exists. | Nothing at runtime; `.env.example:48` still names it — pending `point-env-example-at-higgsfield`, then archive |
