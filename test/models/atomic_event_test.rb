...
class AgentActivityTest < ActiveSupport::TestCase
  ...
  test "[unit] valid with the required attributes present" do
    event = AgentActivity.new(session_id: "sess-1", category: "Explore",
  ...
  test "[unit] session_id is required" do
    event = AgentActivity.new(category: "Explore", reason_slug: "x", opened_at: Time.current)
  ...
  test "[unit] reason_slug is required" do
    event = AgentActivity.new(session_id: "sess-1", category: "Explore", opened_at: Time.current)
  ...
  test "[unit] category accepts every declared category" do
    AgentActivity::CATEGORIES.each do |category|
      event = AgentActivity.new(session_id: "sess-1", category: category,
  ...
  test "[unit] the category vocabulary is exactly the ten declared spans" do
    assert_equal %w[Explore Edit Verify Version Workflow Delegate Clarify Remote Research Plan],
                 AgentActivity::CATEGORIES
  end

  test "[unit] category is required" do
    event = AgentActivity.new(session_id: "sess-1", reason_slug: "x", opened_at: Time.current)
  ...
  test "[unit] category rejects a value outside the vocabulary" do
    event = AgentActivity.new(session_id: "sess-1", category: "Vibe",
                            reason_slug: "x", opened_at: Time.current)
  ...
  test "[unit] open? and closed? reflect closed_at" do
    assert AgentActivity.new(closed_at: nil).open?
    assert_not AgentActivity.new(closed_at: nil).closed?
    assert AgentActivity.new(closed_at: Time.current).closed?
    assert_not AgentActivity.new(closed_at: Time.current).open?
  end
  ...
  test "[unit] scopes filter by session open closed and chronological order" do
    ...
    open_a  = AgentActivity.create!(session_id: "scope-sess", category: "Explore", reason_slug: "a",
                                  seq: 0, opened_at: t0)
    closed  = AgentActivity.create!(session_id: "scope-sess", category: "Edit", reason_slug: "b",
                                  seq: 1, opened_at: t0 + 60, closed_at: t0 + 120)
    AgentActivity.create!(session_id: "other-sess", category: "Plan", reason_slug: "c",
                        seq: 0, opened_at: t0)

    assert_equal 2, AgentActivity.for_session("scope-sess").count
    assert_equal [open_a.id], AgentActivity.for_session("scope-sess").open.pluck(:id)
    assert_equal [closed.id], AgentActivity.for_session("scope-sess").closed.pluck(:id)
    assert_equal [open_a.id, closed.id],
                 AgentActivity.for_session("scope-sess").chronological.pluck(:id)
  end

  test "[unit] next_seq_for is zero for a blank or unseen session and max+1 otherwise" do
    assert_equal 0, AgentActivity.next_seq_for(nil)
    assert_equal 0, AgentActivity.next_seq_for("never-seen")

    AgentActivity.create!(session_id: "seq-sess", category: "Explore", reason_slug: "a",
                        seq: 4, opened_at: Time.current)
    assert_equal 5, AgentActivity.next_seq_for("seq-sess")
  end

  # ---- [integration] open_event! --------------------------------------------

  test "[integration] open_event! opens a span at seq 0 with the given fields" do
    event = AgentActivity.open_event!(
  ...
  test "[integration] open_event! stamps a known acting soul on the span" do
    event = AgentActivity.open_event!(session_id: "agent-sess", category: "Edit",
  ...
  test "[integration] open_event! defaults agent to nil — the base session mascot did it" do
    event = AgentActivity.open_event!(session_id: "agent-sess-3", category: "Explore", reason_slug: "look")

    assert_nil event.agent
  end

  test "[integration] opening a new span auto-closes the prior open span" do
    first = AgentActivity.open_event!(session_id: "auto-sess", category: "Explore", reason_slug: "look")
    second = AgentActivity.open_event!(session_id: "auto-sess", category: "Edit", reason_slug: "change")
    ...
    assert_equal 1, AgentActivity.for_session("auto-sess").open.count,
                 "exactly one span stays open per session"
  end

  test "[integration] open_event! validates BEFORE auto-closing the prior span" do
    open = AgentActivity.open_event!(session_id: "guard-sess", category: "Explore", reason_slug: "keep me")

    assert_raises(ActiveRecord::RecordInvalid) do
      AgentActivity.open_event!(session_id: "guard-sess", category: "Nonsense", reason_slug: "bad")
    end

    assert open.reload.open?, "a rejected open must NOT strand the session with everything closed"
    assert_equal 1, AgentActivity.for_session("guard-sess").open.count
  end
  ...
  test "[unit] open_event! stamps the auto-closed prior span's outcome when given" do
    AgentActivity.open_event!(session_id: "boundary-sess", category: "Explore", reason_slug: "look")
    second = AgentActivity.open_event!(session_id: "boundary-sess", category: "Edit",
                                     reason_slug: "change", prior_outcome_slug: "found the bug")

    prior = AgentActivity.for_session("boundary-sess").order(:seq).first
  ...
  test "[unit] open_event! without a prior_outcome auto-closes prior with a NULL outcome" do
    AgentActivity.open_event!(session_id: "nullout-sess", category: "Explore", reason_slug: "look")
    AgentActivity.open_event!(session_id: "nullout-sess", category: "Edit", reason_slug: "change")

    prior = AgentActivity.for_session("nullout-sess").order(:seq).first
    assert prior.closed?
  ...
  test "[unit] a blank prior_outcome never blanks an outcome a prior close already set" do
    AgentActivity.open_event!(session_id: "keep-sess", category: "Verify", reason_slug: "run tests")
    AgentActivity.close_event!(session_id: "keep-sess", outcome_slug: "already green")
    # There is no open span now; opening the next with a blank prior_outcome must
    # not touch the already-closed span's outcome.
    AgentActivity.open_event!(session_id: "keep-sess", category: "Edit", reason_slug: "next",
                            prior_outcome_slug: "")
  ...
  test "[integration] close_all_open! closes the open span with a shared outcome" do
    AgentActivity.open_event!(session_id: "end-sess", category: "Edit", reason_slug: "mid-edit")

    count = AgentActivity.close_all_open!(session_id: "end-sess", outcome_slug: "session ended")

    assert_equal 1, count
    span = AgentActivity.for_session("end-sess").order(:seq).last
    assert span.closed?
    assert_equal "session ended", span.outcome_slug
    assert_equal 0, AgentActivity.for_session("end-sess").open.count
  end
  ...
  test "[integration] close_all_open! closes EVERY open span, not just the latest" do
    ...
    AgentActivity.create!(session_id: "multi-end", category: "Explore", reason_slug: "a",
                        seq: 0, opened_at: Time.current)
    AgentActivity.create!(session_id: "multi-end", category: "Edit", reason_slug: "b",
                        seq: 1, opened_at: Time.current)

    count = AgentActivity.close_all_open!(session_id: "multi-end", outcome_slug: "session ended")

    assert_equal 2, count
    assert_equal 0, AgentActivity.for_session("multi-end").open.count
  end

  test "[integration] close_all_open! is a no-op returning 0 when nothing is open" do
    assert_equal 0, AgentActivity.close_all_open!(session_id: "never-open-end", outcome_slug: "x")
  end
  ...
  test "[integration] open_event! links to a task via the slug FK" do
    ...
    event = AgentActivity.open_event!(session_id: "fk-sess", category: "Plan",
                                    reason_slug: "shape the model", task_slug: task.slug)

    assert_equal task.slug, event.task_slug
    assert_equal task, event.task
    assert_includes task.agent_activities, event
  end

  # ---- [integration] close_event! -------------------------------------------

  test "[integration] close_event! stamps the outcome and closes the open span" do
    AgentActivity.open_event!(session_id: "close-sess", category: "Verify", reason_slug: "run tests")

    closed = AgentActivity.close_event!(session_id: "close-sess", outcome_slug: "green suite")
  ...
  test "[integration] close_event! closes the LATEST open span" do
    AgentActivity.open_event!(session_id: "latest-sess", category: "Explore", reason_slug: "one")
    AgentActivity.open_event!(session_id: "latest-sess", category: "Edit", reason_slug: "two")

    closed = AgentActivity.close_event!(session_id: "latest-sess", outcome_slug: "done two")

    assert_equal "two", closed.reason_slug, "closes the current (latest) open span"
    assert_equal 0, AgentActivity.for_session("latest-sess").open.count
  end

  test "[integration] close_event! is a no-op when the session has no open span" do
    assert_nil AgentActivity.close_event!(session_id: "never-opened", outcome_slug: "x")
  end
  ...
  test "[integration] concurrent soul spans do NOT close each other" do
    carl    = AgentActivity.open_event!(session_id: "lane", category: "Verify", reason_slug: "backend", agent: "carl")
    shannon = AgentActivity.open_event!(session_id: "lane", category: "Verify", reason_slug: "ui", agent: "shannon")
    ...
    assert_equal 2, AgentActivity.for_session("lane").open.count, "both lanes open at once"
  end

  test "[integration] opening a new span for a soul auto-closes only THAT soul's prior span" do
    carl_first  = AgentActivity.open_event!(session_id: "lane2", category: "Explore", reason_slug: "scan", agent: "carl")
    shannon     = AgentActivity.open_event!(session_id: "lane2", category: "Verify", reason_slug: "ui", agent: "shannon")
    carl_second = AgentActivity.open_event!(session_id: "lane2", category: "Edit", reason_slug: "fix",
  ...
  test "[integration] close_event! closes only the named soul's lane" do
    carl    = AgentActivity.open_event!(session_id: "lane3", category: "Verify", reason_slug: "backend", agent: "carl")
    shannon = AgentActivity.open_event!(session_id: "lane3", category: "Verify", reason_slug: "ui", agent: "shannon")

    closed = AgentActivity.close_event!(session_id: "lane3", agent: "carl", outcome_slug: "approve: clean")
  ...
  test "[integration] the orchestrator's nil lane is independent of soul lanes" do
    orchestrator = AgentActivity.open_event!(session_id: "lane4", category: "Plan", reason_slug: "orient")
    carl         = AgentActivity.open_event!(session_id: "lane4", category: "Verify", reason_slug: "review", agent: "carl")

    # closing the nil lane (no agent) leaves the soul lane open, and vice versa
    AgentActivity.close_event!(session_id: "lane4", outcome_slug: "planned")
  ...
  test "[integration] close_all_open! still closes EVERY lane at session end" do
    AgentActivity.open_event!(session_id: "lane5", category: "Plan", reason_slug: "orient")
    AgentActivity.open_event!(session_id: "lane5", category: "Verify", reason_slug: "a", agent: "carl")
    AgentActivity.open_event!(session_id: "lane5", category: "Verify", reason_slug: "b", agent: "shannon")

    count = AgentActivity.close_all_open!(session_id: "lane5", outcome_slug: "session ended")

    assert_equal 3, count, "the session-end teardown closes all three lanes"
    assert_equal 0, AgentActivity.for_session("lane5").open.count
  end

  test "[unit] normalize_agent_value maps known souls and coerces the rest to the nil lane" do
    assert_equal "carl", AgentActivity.normalize_agent_value("Carl")
    assert_nil AgentActivity.normalize_agent_value("nobody")
    assert_nil AgentActivity.normalize_agent_value("")
    assert_nil AgentActivity.normalize_agent_value(nil)
  end

  test "[integration] an unknown close agent resolves the nil lane, not a soul lane" do
    orchestrator = AgentActivity.open_event!(session_id: "lane6", category: "Plan", reason_slug: "orient")
    AgentActivity.open_event!(session_id: "lane6", category: "Verify", reason_slug: "review", agent: "carl")

    # a typo'd/unknown agent normalizes to the nil lane (matches the record coercion)
    closed = AgentActivity.close_event!(session_id: "lane6", agent: "typo", outcome_slug: "planned")
  ...
  test "[integration] destroying a span nullifies its actions rather than destroying them" do
    event = AgentActivity.open_event!(session_id: "orphan-sess", category: "Explore", reason_slug: "look")
    action = AgentAction.capture(session_id: "orphan-sess", kind: "read")
    assert_equal event.id, action.agent_activity_id

    assert_no_difference -> { AgentAction.count } do
      event.destroy!
    end
    assert_nil action.reload.agent_activity_id, "the raw action survives as orphaned history"
  end

  # ---- [integration] awaiting_grade — the grade-events READ path -------------

  def resolved_span(session_id:, reason: "resolved span here", **attrs)
    AgentActivity.open_event!(session_id: session_id, category: "Verify", reason_slug: reason, **attrs)
  ...
  test "[integration] awaiting_grade clamps the limit" do
    ...
    assert_equal 1, AgentActivity.awaiting_grade(limit: 0).size, "0 clamps up to 1"
    assert_operator AgentActivity.awaiting_grade(limit: 999).size, :<=, AgentActivity::MAX_GRADE_BATCH
  end

  test "[unit] to_grading_row carries what a grader needs and drops nils" do
    span = AgentActivity.open_event!(session_id: "row-sess", category: "Verify",
  ...
  test "[integration] close_event! stamps the key method with an inferred lang" do
    AgentActivity.open_event!(session_id: "km-sess", category: "Edit", reason_slug: "add the guard")
    closed = AgentActivity.close_event!(session_id: "km-sess", outcome_slug: "guard added",
  ...
  test "[integration] a bare close never blanks a key method the span already carries" do
    AgentActivity.open_event!(session_id: "km-keep", category: "Verify", reason_slug: "run suite")
    AgentActivity.close_event!(session_id: "km-keep", key_method: "bin/rails test", key_method_lang: "bash")
    reopened = AgentActivity.for_session("km-keep").order(:seq).last
    reopened.update!(closed_at: nil)

    closed = AgentActivity.close_event!(session_id: "km-keep", outcome_slug: "green")
  ...
  test "[integration] open_event! with prior_key_method stamps the auto-closed prior span" do
    prior = AgentActivity.open_event!(session_id: "km-next", category: "Explore", reason_slug: "orient")
    AgentActivity.open_event!(session_id: "km-next", category: "Edit", reason_slug: "make the change",
                            prior_outcome_slug: "seam found",
                            prior_key_method: "grep -rn AgentActivity app/models")
