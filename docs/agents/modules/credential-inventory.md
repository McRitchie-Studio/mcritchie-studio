...
| `agent.solana` | `studio-agents` | Legacy Xan Solana wallet `F6f8…` (the identity was called "Alex Bot" until 2026-09-15); retired as a SIGNER after the 2026-06-02 key rotation and superseded by `agent.xan.solana` below. Carries no `private key` field (verified 2026-09-14), so it cannot sign anything. **KEPT ON PURPOSE** — it backs seed data and devnet test fixtures, so it is not a deletion candidate; what is retired is its place on a multisig, and it must never be put back on one. | Seed + devnet test use |
| `agent.xan.solana` | `studio-agents-admin` | The **Xan** signer (`8K81…`) — the rotated replacement for `agent.solana` above, renamed from `agent.alex.solana` and MOVED out of `studio-agents` on 2026-09-15. NOT the human Alex Phantom wallet (`7ZDJ…`), which has no item in any vault. Read it under `source ~/.zprofile.admin`; the field label is SPACED (`private key`), verified 2026-09-15. Consumers that still name the OLD identity are turf-vault code, not stale prose: `squad.json` → `members.alex_bot` and `squad-upgrade.js` → `ALEX_BOT_KEY`. **⚠ `squad-upgrade.js` IS NO LONGER A WORKING UPGRADE PATH.** The Squads config transaction of 2026-09-15 09:41 REMOVED `8K81…` from both multisigs, so the key this script signs with can no longer initiate, vote or execute there — measured on-chain at `finalized`. `squad.json`'s `members` block still lists the old three and is now provenance only. **What this item IS still good for:** it is the value of `SOLANA_ADMIN_KEY` on `turf-monster-mainnet`, and `8K81…` is still a live `VaultState.signers` entry (2-of-3, unchanged on both clusters) — a DIFFERENT authority from the Squads multisig, which is why one moved and the other did not. It is NOT the agent's Turf Monster admin identity; that is `solana.turf.admin` below. | Production `SOLANA_ADMIN_KEY`; `VaultState` cosign. **Not** turf-vault upgrades |
| `agent.mason.solana` | `studio-agents` | Mason's vault signer (`CytJ…`) — **no longer a Squads V4 member**, still a `VaultState.signers` entry. The 2026-09-15 09:41 config transaction removed him from BOTH multisigs alongside Xan (measured on-chain at `finalized`), so `squad-upgrade.js`'s `MASON_KEY` second approval cannot land either. He remains one of the three live `VaultState.signers` (2-of-3, unchanged), which is a separate authority — see the two-authority note under `solana.turf.admin`. | `VaultState` cosign. **Not** Squads approvals |
...
| `solana.turf.admin` | `studio-agents` | **The agent governance identity** — `BLSBw8fXHzZc5pbaYCKMpMSsrtXBTbWXpUPVzMrXx9oo`, item id `2xrfvfho2txchqtem565wmmmfu`. Agent-readable, which is the point: both of the agent's keys now sit in a vault it can open, which is honest rather than merely restricted. Field labels are HYPHENATED (`wallet-address`, `private-key`) where the older `agent.*` items use spaces — verified 2026-09-15 by deriving the public key from the stored private key. Same wallet the old `agent.turf.solana` filed; the item was recreated, the identity was not. **It now holds the agent's ONE Squads seat.** As of the 2026-09-15 09:41 config transaction both multisigs (mainnet `4H3fP3ot…`, devnet `7nRuVw3V…`) are **3-of-4**: `3Qj4v9…`, `7ZDJ…`, `9gACbz…` and this key, all mask 7. **TWO SETS THAT DIFFER BY ONE WALLET, BY DESIGN** — Squads (program UPGRADE authority) is FOUR and live; `VaultState.signers` (treasury/governance — the money) is FIVE and is the target, still the untouched 2-of-3 (`8K81…`, `7ZDJ…`, `CytJ…`) on-chain until `update_signers` runs. The difference is `solana.turf.system`, which is on the vault and deliberately off Squads. This key is in BOTH. A change to one authority is never a change to the other; say *Squads* or *`VaultState`* and never "the multisig" — see **Onchain Admin** in `credentials.md` for the full trade. ⚠ **`3Qj4v9…` and `9gACbz…` are Mr. McRitchie's PERSONAL wallets — already seated, and deliberately filed in NO vault. Never file them into an agent-readable vault to unblock a ceremony.** The agent holding exactly one of four seats is the separation this buys. | `turf_monster/qa_rehearsal/key_store.rb` (`turf-admin`); `bin/ecosystem-build` fills local `SOLANA_ADMIN_KEY` from it; the agent's Squads seat |
| `solana.turf.system` | `studio-agents` | **The server operational key, MAINNET** — `7auwTLSvNniSUeAgL6v9RStMXJhWrrUhSJgwFWLpcqC`, item id `hvt5htkgjqsilq5blv3uztqie4`. Hyphenated labels. ⚠ **Held 0 SOL on mainnet as of 2026-09-15.** It is the intended future value of `SOLANA_ADMIN_KEY` on `turf-monster-mainnet`, but repointing an underfunded fee payer breaks production settlement, so that cutover is a separate task gated on funding. **DELIBERATELY NOT A SQUADS MEMBER**, and this is the one asymmetry between the two signer sets: it belongs to `VaultState.signers` (slot 1 of the five-member target) and to no multisig. It is the app's HOT key — it sits in Heroku config on a running dyno and signs every entry and payout — so it is the most exposed key here and the last that should hold program upgrade authority. Squads is FOUR members; `VaultState` is FIVE; the difference is this wallet. | Planned `turf-monster-mainnet` `SOLANA_ADMIN_KEY`; `VaultState` cosign. **Never** Squads |
