...
module Api
  module V1
    class AgentActivitysControllerTest < ActionDispatch::IntegrationTest
      ...
      # ---- [integration] turn_open is NEUTRALIZED (retire-turn-auto-open-spans) --
      # Auto-open produced only "(working)" noise (chat filler, 0 actions); agent-authored
      # narration is the primary path. The endpoint no longer opens a span — 204, zero
      # spans, regardless of preamble/turn_uuid. The follow-up measure-only meter
      # repurposes this seam to DETECT orphans, not create spans. (open_for_turn! itself
      # is left intact + still unit-tested for that reuse.)

      test "[integration] turn_open no longer opens a span — 204 no-op" do
        assert_no_difference -> { AgentActivity.count } do
          post turn_open_api_v1_agent_activities_path,
               params: { session_id: "to-neutralized", turn_uuid: "t-1", mascot: "charmander",
                         preamble: "Found it. Now let me check the table." },
               headers: @headers, as: :json
        end

        assert_response :no_content
        assert_equal 0, AgentActivity.for_session("to-neutralized").count
      end
    end
  end
end
