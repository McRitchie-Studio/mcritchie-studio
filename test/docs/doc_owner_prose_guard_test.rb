...
class DocOwnerProseGuardTest < ActiveSupport::TestCase
  ...
  ACT_OWNER = {
    ...
    "roster-sync" => "Turf Monster",  # registered 2026-09-21 WITH the SOP, while that SOP is still PENDING — deliberately. This table is not an invocation registry: it is the only thing that can catch a WRONG-SOUL claim in prose, and the comment above calls an absent act "the quiet failure mode — the guard reports clean because it was never asked". A Pending act can still be written about wrongly. The HEARTBEAT row is the one withheld until the SOP goes Active; that one does route invocations
