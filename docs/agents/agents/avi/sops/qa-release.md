...
1. Detect every `reviewed` task plus any `assembled` straggler. A task naming a
   **parked** repo — any `config/release_repos.yml` ladder but `three-rung`
   (rolio `dormant`, tax-studio `planned`, chain-ops `blocked`) — is HELD, not
   swept: `prepare` prints `⚠ HELD <slug>: names parked <repo> (ladder: <ladder>)`,
   leaves the task in its stage, and never promotes, records or deploys it. The
   rest of the sweep carries on. A task naming a live repo AND a parked one is
   held WHOLE, because shipping only the live half would stamp it assembled and
   shipped for a repo that never moved.
...
4d. **Allocate gem versions, publish gem members, bump consumer locks — BEFORE
   ...
   **The changelog roll.** A swept gem is a `gems:` entry in
   ...
   - **SKIP** — nothing allocated, nothing rolled. Prepare prints `gem <repo>:
     ...
     compares the `version_file` at `origin/accepted` — the rung phase 0 writes —
     with the last `v*` TAG, which is read from `origin/release` because that is
     where publish tags sit (the
     ...
     tag push lands, the version equals its tag. Three things reach it:
   ...
   - **REFUSE** — the decide phase stops the sweep before phase 0 writes
     ...
     below), including a version already LIVE on RubyGems whose `v*` tag never
     reached origin: a publish whose tag push failed. That once SKIPped as
     allocated already, silently and for good, because the stranded-work guard
     fires only when the version did NOT advance. The failing sweep itself
     carries on (its gem is live) and prints `⚠ tag v<version> did NOT reach
     origin — push it now: …`; push it then, or the next sweep from a clone
     without the tag refuses. `Release::Changelog.refusal`, read only for an
     ALLOCATE, refuses a changelog the roll would misfile (the CHANGELOG
     BACKLOG row below).
   ...
   may already sit on `origin/accepted`. Leave it THERE: the re-run reads
   `origin/accepted`, SKIPs it as allocated already, and the batch promote
   carries it to `release`. **Do not tidy it onto `release` instead.** A version
   commit on `release` that `accepted` does not contain is the MIXED state:
   allocation reads `current` from `accepted` while its tag floor comes from
   `release`, so `tag..accepted` comes back empty and the next run prints
   `no commits past the last published tag — nothing to publish` and skips
   SILENTLY — nothing refuses that state today
   (/tasks/carry-release-commit-onto-accepted).
...
- **ABORT** — `prepare` reached a verdict and refused: a red pre-QA gate, a failed
  QA boot, a QA deploy whose run never registered or could not be read, a failed
  merge. **Fix the cause first, THEN re-run.**
- **NOT GREEN** — `prepare` reached a verdict, printed it, and **returned
  normally**. QA did not come up, so nothing was assembled; the release stays
  `assembling` and its members stay `reviewed`. **Fix the cause first, THEN
  re-run**, exactly as for an abort.
...
**THE EXIT CODE IS NEVER THE VERDICT.** A NOT-green `prepare` exits **0** — so a
wrapper printing `PREPARE EXIT: 0` says nothing about whether QA came up, and on
2026-09-07 that zero was read as success over a sweep that assembled **0 members**.
Read the run's own summary line: `✓ Assembled <rel>` is green; `✓ Prepared (NOT
assembled — QA not green)` is not. The NOT-green block now says so itself, next to
the failure.
...
|---|---|---|
| **A GEM'S `version_file` MOVED AND THE SWEEP ABORTS EITHER WAY** (either "gem <repo>: <version_file> does not declare EXACTLY ONE version literal" — **phase 0b, the WRITE, not the phase-0 refusal one row down** — OR "promote refused — <repo> (suite workflow …) cannot certify `accepted`") | **The registry and the gem tree must move TOGETHER, and `bin/release.rb` reads the registry from the CONDUCTOR'S OWN checkout** (`bin/release.rb#RELEASE_REPOS`) — not from the candidate. So a self-shipping RC driven from `origin/main` reads the OLD registry against a NEW gem tree. Both mismatches fail closed and neither publishes, but **they abort in different places and say different things**, which is what makes this hard to look up:<br>**OLD registry + NEW gem tree** → allocation SUCCEEDS (it derives 0.5.0 quite happily); the failure lands one phase later, in `commit_gem_version!` — at its `rewrite_version` refusal, the one that names the `version_file` and prints the set-it-by-hand remedy — when `rewrite_version` finds no literal to rewrite. (Named by SEAM, not by line: every commit that touches `bin/release.rb` moves its line numbers, and a sample of eight `bin/release.rb:<line>` citations across this repo on 2026-09-14 found **all eight already pointing at unrelated lines**.) Its message interpolates the `version_file` — so it names the **gemspec**, which is misleading once the version has moved out of it. **⚠ Do not follow that abort's own remedy here.** It says "set it to <version> by hand and commit it onto `accepted`", which in THIS case means hardcoding a literal back into the gemspec — re-introducing the very coupling the move exists to remove, and hand-setting a version the row below rightly warns against. The fix is to land the hub registry change.<br>**NEW registry + OLD gem tree** → the **promote** aborts in `refuse_blind_accepted!`, taking every repo in the candidate with it. This one never mentions the gemspec: it names the repo and its suite workflow, and **reads identically to the A REPO CANNOT CERTIFY `accepted` row below**. Do not apply that row's remedy here. The difference is that here the workflow is **already correct** (`gem-ci.yml` declares `push: branches: [accepted, release, main]`) and it is the GEM TREE that has not landed — editing `on.push.branches` changes a correct file and leaves the real cause untouched.<br>**Only NEW+NEW works.** Land the gem-side move and the `config/release_repos.yml` change in the SAME release, gem repo first (its workflow must be on the gem's `accepted` before the map points at it), and drive the sweep from a checkout carrying the new registry.<br>**AND EXPECT THE BUILDER'S GATE TO LOOK CIRCULAR WHILE YOU DO.** `bin/dor-check` reads the registry from ITS OWN checkout too (`bin/dor-check#gem_version_files`), so until the hub change lands, the gem PR's edit to the old version_file stays REFUSED — the gem must merge first for the sweep, yet cannot clear dor-check until the hub half exists. It is not a deadlock: run dor-check from a checkout that already carries the registry change (the hub worktree holding it), and land the two together | land both, then re-run `prepare`; nothing was published |
...
| **GEM VERSION ALLOCATION REFUSED** (step 4d phase 0 — "REFUSING to allocate a version") | **Do not set a version by hand to route around this.** The refusal names its own cause and each has a one-line fix: an unreadable override → `bin/task update <task> --gem-bump patch\|minor\|major` (or clear it); an unparseable last published version → fix the gem's `v*` tag or its `version_file` by hand; a `version_file` declaring its version twice → make it declare one; `bundle lock` failed or left the lock on the old version → fix the bundle in the gem repo (a stale resolution is usually RubyGems propagation — wait, as in the CONSUMER LOCK BUMP row below); `<version> is already live on RubyGems, but the last v* tag…` → the tag push of that publish failed: push it from the clone that published it (`git -C /Users/alex/projects/<gem-repo> push origin v<version>`), or tag the `Release <version>` commit on `origin/release` and push that. A REFUSE writes nothing. The `version_file` and `bundle lock` causes are write-phase failures, so an earlier gem's version commit may already sit on `accepted`: leave it THERE, because the re-run reads `accepted`, SKIPs it as allocated already, and the batch promote carries it to `release` — moving it onto `release` alone creates the MIXED state that makes the next run skip silently (step 4d) | re-run `prepare`; allocation resumes |
...
| **QA DEPLOY DISPATCHED, RUN LIST UNREADABLE** (step 6 — "`<workflow>` was dispatched and the run list could NOT be read afterwards — whether a run was created is UNKNOWN") | **This abort reports what was OBSERVED, and the run list would not answer.** The post-dispatch `gh run list` was still FAILING when the poll gave up — an earlier read in the same poll may well have answered, which is exactly why the abort turns on the LAST one and not on whether any read succeeded — so the deploy may be running right now — the abort deliberately does not claim otherwise. **Do NOT re-dispatch on the strength of this message**: a blind re-dispatch can land a second deploy on top of a live one, and against `prod-deploy.yml` that is a second PRODUCTION deploy. Fix the reader first — `gh auth status`, and the token is self-service (`eval "$(bin/gh-auth-refresh --export)"`) — then LOOK: `gh run list --workflow <workflow> --limit 5`. **If a run for that SHA is listed, WATCH it** (`gh run watch <id> --exit-status`) and let it finish; only a readable list with no such run makes the row above's hand-dispatch safe. As in that row, the promote and the gem publish have already happened | once the deploy's real state is KNOWN: re-run `prepare` (it resumes over the promoted/published work) |
| **QA DEPLOY NOT DISPATCHED — NO BASELINE** (step 6 — "`<workflow>`: `gh run list` never answered/FAILED, so there is no baseline to tell our run from a prior one — NOT dispatching. NOTHING WAS DEPLOYED" — it says **never answered** when gh printed nothing and **FAILED** when gh answered with an error, and the error itself is quoted underneath) | **The refusal is CORRECT — there is no way to dispatch safely from here.** Without a pre-dispatch snapshot the sweep cannot tell its own run from a prior one, so it would watch someone else's verdict. **Read the lines underneath it**: the refusal now quotes gh verbatim (`gh said:`) and the `→` remedy is chosen FROM those words — a `HTTP 401: Bad credentials` prints the re-mint recipe, not a generic retry. The snapshot retries by CAUSE, so reaching this row means the recovery was already tried: a credential-class refusal re-mints once and retries IMMEDIATELY (you will have seen `↻ <workflow>: the ambient GitHub credential was refused — re-minted and continued.`), and every other failure keeps the bounded sleep-retry. **A sleep was never the remedy for the common cause** — `bin/release` inherits `GH_TOKEN` from the shell that launched it and App installation tokens live ~1h BY DESIGN, so a long sweep outlives its own credential; measured twice on 2026-09-22 (rel-20260922-7210bd, rel-20260922-a299ae), where five blind reads three seconds apart cleared neither and a hand re-run cleared both. **The promote and any gem publish have ALREADY happened** (as in the two rows above) — this is not a clean slate | fix what gh named, then re-run `prepare`; it resumes over the merged/published work |
| **QA deploy / boot FAILED** ("never returned /up 200") | Fix the boot failure (the summary prints the `bin/qa-server deploy …` retry); eject the member if it is the cause. **FIRST confirm the app was actually deployed to — this row does not prove it.** A dispatch whose run never registered aborts at the rows above, but two dispatch failures still land here as a boot verdict: `gh run list` never answering (so prepare never dispatched at all — **its own row is directly above**), and a `gh workflow run` that gh itself refused. Both now print `⚠ <workflow>: … NOTHING WAS DEPLOYED; this is not a boot failure` ABOVE the summary — **scroll up and look for that line before touching the app.** It matters because `qa_ok &&=` short-circuits: on those two paths the `/up` poll is never issued, so "never returned /up 200" describes a request that was never made | re-run `prepare` **once QA boots** |
...
| **CHANGELOG MISFILE GUARD REFUSED THE PROMOTE** (step 3 — "the CHANGELOG misfile guard REFUSED the promote — NOTHING was promoted, recorded or deployed", then per gem "CHANGELOG.md would file N line(s) written under '## Unreleased' beneath a version that already shipped without them" or "the promote would CONFLICT in CHANGELOG.md") | **Nothing was promoted in ANY repo**: the guard reads every swept gem before the first `gh pr merge`. The roll now lands on `accepted` and reaches `release` through this same promote, so a completed sweep leaves both rungs agreeing about which entries shipped; the guard stays armed for a gem whose `accepted` did NOT absorb a `Release <version>` commit, because git merges a bullet added inside an existing `###` subsection CLEANLY under the heading the roll wrote. Prepare predicts each gem's promote with `git merge-tree` and refuses when a line either side added to `## Unreleased` would land under a version that already has a `v*` tag, or when the promote would conflict in `CHANGELOG.md`. The fix is the one the refusal prints: merge `origin/release` into a branch off the gem's `accepted`, move every line the merge filed under a shipped version back under `## Unreleased` (resolve a conflict the same way), and push that merge straight onto `accepted` — **not a PR**, which `bin/dor-check` refuses because it carries the release's version bump. Three fail-closed variants name no line ("git fetch failed", "could not resolve origin/release and origin/accepted", "git merge-tree could not predict the promote", which needs git 2.38+): fix what they name. The merge is then pinned to the checked head (`--match-head-commit`), so a commit landing on `accepted` in between makes `gh pr merge` fail: re-run | re-run `prepare`; it resumes |
...
| **`⚠ HELD <slug>: names parked <repo> (ladder: <ladder>)`** (step 1 — not an abort; the rest of the sweep proceeds. `bin/release merge <slug>` REFUSES the same task, because there you named it) | The task names a repo the conductor does not sweep. **Do not force it through** — there is no flag for that, deliberately. Decide which fact is wrong: if the repo carries no work for this task, drop it from `devops.repositories` (`bin/task update <slug> --repo …`, which REPLACES the list); if the repo is being revived, give it both rungs and set `ladder: three-rung` on its registry row in a task of its own. A mixed task holds its live repo too, so dropping the parked repo is what frees the live half. Nothing was promoted, recorded or deployed for it. A parked repo that still reaches the deploy plan is REFUSED at step 3b (`parked repo in the deploy plan`) — same fix | re-run `prepare`; the task sweeps once no repo it names is parked |
