...
# token/cost/model of the activity) and two grade editors (Xan + the McRitchie audit)
...
class HeartbeatEventDrawerTest < ActionView::TestCase
  ...
  test "[component] the span drawer renders Xan + McRitchie editors posting to the E2 endpoint" do
    ...
    render partial: "heartbeat/activity_drawer",
           locals: { event: ev, actions: [a1], xan: nil, mcr: nil }
    ...
    assert_select "form[data-grader=xan]"
    assert_select "form[data-grader=mcr]"
    # the two graders are carried as hidden fields
    assert_select "form[data-grader=xan] input[name=grader][value=xan]"
  ...
  test "[component] only Xan's span editor carries the bank/discard controls" do
    ev = event(seq: 0, closed_at: Time.current, outcome_slug: "done")

    render partial: "heartbeat/activity_drawer",
           locals: { event: ev, actions: [], xan: nil, mcr: nil }

    # Xan banks; the McRitchie audit does not
    assert_select "form[data-grader=xan] button[value=bank]", 1
  ...
  test "[component] an existing span grade pre-fills its editor slug" do
    ev = event(seq: 0, closed_at: Time.current, outcome_slug: "done")
    xan = ActionGrade.create!(agent_activity: ev, grader: "xan", disposition: "not",
                               slug: "span drifted off its stated reason")

    render partial: "heartbeat/activity_drawer",
           locals: { event: ev, actions: [], xan: xan, mcr: nil }

    assert_select "form[data-grader=xan] input[name=slug][value=?]", "span drifted off its stated reason"
  end
end
