...
# — every lifecycle timestamp, in logical progress order — so a dropped or
# mis-ordered column in the CREATE VIEW SQL fails loudly.
class TimelineViewsTest < ActiveSupport::TestCase
  ...
  test "[integration] the views select live rows from their base tables" do
    # A smoke that the view is a real, queryable projection (not just DDL).
