...
Each agent has a dedicated Solana wallet. Credentials stored in 1Password. The three vault-admin identities below (Xan, Alex's Phantom, Mason) are **the same keys on devnet and mainnet** — verified 2026-09-05 as the `VaultState.signers` set on both clusters — so a rotation of any of them is a mainnet event, not a devnet one. The other rows were not part of that verification: check the cluster before assuming any of them is devnet-only.
...
| Alex's Phantom | `7ZDJp7FUHhuceAqcW9CHe81hCiaMTjgWAXfprBM59Tcr` | Backup vault admin (recovery only) |
...
> and awaiting a deletion only Alex can perform (the agent service
...
1. **Restore a working upgrade path — DONE 2026-09-15** (turf-vault `280cebf`, on `accepted`, `release` and `main`). `scripts/squad-upgrade.js` now requires `--cluster=devnet|mainnet`, dry-runs unless `--send` arms it, reads the live Squads membership on every run, and signs only with the seats in `scripts/lib/squad-clusters.js` `AGENT_SEATS` that the chain still seats. On devnet its three seats (`2eGs8G3w…`, `BLSBw8…`, `8K81…`) reach threshold 3, so it runs end to end. On mainnet its two seats (`7auwTL…`, `BLSBw8…`) cast their approvals and it stops for Alex. It no longer loads `ALEX_BOT_KEY` or `MASON_KEY`; `scripts/tests/squad-upgrade-signers.test.js` pins that.
...
Attempting step 4 first does not fail harmlessly — it spends a ceremony and Alex's signatures on a transaction the live program cannot accept.
