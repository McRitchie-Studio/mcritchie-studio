...
class DorCheckTest < Minitest::Test
  ...
  # ==== [integration] `docs` IS EARNED FROM THE DIFF ==================================
  #
  # THE BUG (/tasks/docs-shape-admits-test-code, 2026-09-02): `shape: docs` carries
  # zero tiers AND `full_suite_gate: false`, and until this landed both were unlocked
  # by TYPING `docs`. The diff was never consulted, so a docs-claimed change carrying
  # executable code shipped with no tier and no cert demanded.
  #
  ...
  # mitigation: a gate satisfied by volunteered evidence holds for careful builders
  # and fails silently for everyone else.
  #
  # WHAT MADE IT SURVIVE was a fall-through, and these tests pin the seam it fell
  # through. The exempt-KIND gate already asks this exact question on this exact
  # diff and already refuses — then hands control to the shape contract, which
  # granted the same exemption one step later. Two answers, one question. So the
  # claim guard reuses CodeDiff, the classifier the kind gate runs, rather than a
  # second prose list that could drift back apart.
  #
  # NOT fixed by `full_suite_gate: true` on docs, deliberately: that would make every
  # prose correction pay a full suite, and push people to mislabel shapes — strictly
  # worse than the hole. Gate the CLAIM, not the cost.
