...
The CNAME was deleted from the zone on 2026-09-03 — via the Squarespace DNS
panel, which fronts this zone's Google-infrastructure nameservers
(`ns-cloud-d1..d4.googledomains.com`), so no separate Google credential was
needed — and a lookup now returns nothing at all. There is likewise no Heroku
app named `turf-monster`; production is `turf-monster-mainnet`.
...
Retired is the decision; the record's removal (2026-09-03) was the last of the
paperwork. Whether an old cache still returns the dead CNAME or a lookup returns
nothing at all, the reading is the same: this host never serves Turf Monster and
is never a valid smoke-test target. Do not recreate or repoint it.
