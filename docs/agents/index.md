...
4. If the work is a feature, bug, QA, release, cleanup, or active-doc change,
   ...
   `devops["test_plan"]`, and `devops["worktree_slug"]`. The production task
   slug is immutable and generated by the app; use `devops["worktree_slug"]`
   for the human-readable feature handle. Move the task to `in_progress` once
...
9. Run `bin/dor-check <task-slug>` and resolve anything it flags. Then commit
   ...
   `pr_review`. Handoffs should include the task URL before the PR URL. Deploy
...
pr_review for Avi QA. Do not merge or deploy unless I explicitly assigned that
lane.
```
...
code or active-doc edits start. `Task.slug` stays the app's immutable generated
id; record the human-readable feature handle in
`metadata["devops"]["worktree_slug"]` and bind the task URL to the worktree.
