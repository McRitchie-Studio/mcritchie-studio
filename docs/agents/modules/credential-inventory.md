...
| `serper.studio.agents` | — (not filed) | **DOES NOT EXIST YET.** Named here so the row exists the day it is filed, and so nobody re-derives the name. Image search (serper.dev, Google Images over REST) for character-model reference photos. One field, `api-key`, read straight from `ENV["SERPER_API_KEY"]` — the house pattern beside `HIGGSFIELD_API_KEY`, no `Rails.credentials` hop. **Its absence is a SUPPORTED state, not a gap to fix in a hurry**: `Appearances::ImageSearch` degrades to the cached-headshot floor and the page says so, naming this variable. ⚠ **The serper.dev response shape is UNVERIFIED** — nothing has ever been driven against the live API, so the first job the day this lands is to probe ONCE, capture the body, and replace the `assumed_serper_body` fixture in `test/services/appearances/image_search/serper_test.rb`. It bills PER QUERY, so a probe is a purchase. | McRitchie Studio character-model page (`Appearances::ImageSearch::Serper`); `rake appearances:search_reference_photos` |
