...
#   ROUND 3  `--grep-invert '@quarantine|board'`   the SANCTIONED exclusion, widened in one
...
class E2eExecutedSet
  ...
  def skipped_failures
    ...
    ["#{skipped_tests.size} spec(s) were SKIPPED AT RUNTIME:\n#{titles}\n" \
     ...
     "it, or ` @quarantine` it (which makes you account for it in config/e2e_lane.yml) and " \
     "BLOCK on /tasks/repair-rotted-e2e-specs."]
  end

  # The exclusion did what it says on the tin, and ONLY that. A quarantined spec must not run
  # (it is red, it would break the lane); an executed spec must not carry the tag.
  def quarantine_leak_failures
    leaked = executed_tests.select { |test| test["title"].to_s.include?(quarantine_tag) }
