# `pr-review-sop.md` before the 300-line cut — archived 2026-09-25

Frozen record. On 2026-09-25 (epic devops-v3, piece 5b-ii) `docs/agents/modules/pr-review-sop.md` was cut to a
module page of at most 300 lines: procedure first, rationale and incident
history out. Nothing was dropped: the file is kept below exactly as it stood at
the cut (444 lines), so every passage the live page no longer carries is here
verbatim. Relative links below resolve from the file's original folder, not this one.

Do not edit this file to keep it current. The live rules are in `docs/agents/modules/pr-review-sop.md`.
