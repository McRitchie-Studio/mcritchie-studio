...
| `agent.xan.solana` | `studio-agents-admin` | The **Xan** signer (`8K81…`) — the rotated replacement for `agent.solana` above, renamed from `agent.alex.solana` and MOVED out of `studio-agents` on 2026-09-15. NOT the human Alex Phantom wallet (`7ZDJ…`), which has no item in any vault. Read it under `source ~/.zprofile.admin`; the field label is SPACED (`private key`), verified 2026-09-15. Consumers that still name the OLD identity are turf-vault code, not stale prose: `squad.json` → `members.alex_bot` and `squad-upgrade.js` → `ALEX_BOT_KEY`. **STILL LOAD-BEARING, AND NARROWER THAN IT WAS.** It is what `squad-upgrade.js` signs with and what `SOLANA_ADMIN_KEY` holds on `turf-monster-mainnet`. It is NO LONGER the agent's Turf Monster admin identity — that is `solana.turf.admin` below, as of 2026-09-15. Mr. McRitchie decided that day to retire `8K81…` from the signer set entirely, but **nothing has been transacted**, so it remains on-chain and in prod config until a Squads config transaction replaces it. | turf-vault upgrades; production `SOLANA_ADMIN_KEY` |
| `agent.mason.solana` | `studio-agents` | Mason's vault signer (`Cyt…`) — a Squads V4 member and a `VaultState.signers` entry | `squad-upgrade.js` reads it as `MASON_KEY` for the second approval |
