...
class ActionGradeTest < ActiveSupport::TestCase
  ...
  def action(session_id: "grade-sess", **attrs)
    AgentAction.capture({ session_id: session_id, kind: "edit", outcome: "ok" }.merge(attrs))
  end
  ...
  def event(session_id: "grade-ev-sess", **attrs)
    AgentActivity.create!({ session_id: session_id, category: "Explore",
  ...
  test "[unit] a grade with no target (neither action nor event) is invalid" do
    grade = ActionGrade.new(valid_attrs(agent_action: nil))

    assert_not grade.valid?
    assert_includes grade.errors[:base], "must grade exactly one of an action or an activity"
  end
  ...
  test "[unit] the same grader may grade a different action" do
    one = ActionGrade.create!(valid_attrs(agent_action: action(session_id: "a-sess")))
    two = ActionGrade.new(valid_attrs(agent_action: action(session_id: "b-sess")))
  ...
  test "[unit] for_action scopes to a single action" do
    ...
    grade = ActionGrade.create!(valid_attrs(agent_action: mine))
    ActionGrade.create!(valid_attrs(agent_action: other))
  ...
  test "[integration] an action carries Alex's grade and the McRitchie audit as two rows" do
    ...
    mcr = ActionGrade.create!(
      agent_action: graded, grader: ActionGrade::MCR,
  ...
  test "[unit] valid when targeting an event span instead of an action" do
    ...
    assert grade.valid?, grade.errors.full_messages.to_sentence
    assert grade.event_grade?, "a grade with an agent_activity_id targets a span"
  end

  test "[unit] targeting BOTH an action and an event is invalid (XOR)" do
    grade = ActionGrade.new(valid_attrs(agent_activity: event))

    assert_not grade.valid?
    assert_includes grade.errors[:base], "must grade exactly one of an action or an activity"
  end

  test "[unit] an event grade persists with a null agent_action_id" do
    grade = ActionGrade.create!(event_valid_attrs)

    assert grade.persisted?
    assert_nil grade.agent_action_id
    assert grade.agent_activity_id.present?
  end
  ...
  test "[unit] the same grader may grade both an action and an event" do
    act  = ActionGrade.create!(valid_attrs(agent_action: action(session_id: "cross-act")))
    span = ActionGrade.new(event_valid_attrs(agent_activity: event(session_id: "cross-ev")))
  ...
  test "[unit] for_event scopes to a single event" do
    ...
    grade = ActionGrade.create!(event_valid_attrs(agent_activity: mine))
    ActionGrade.create!(event_valid_attrs(agent_activity: other))
  ...
  test "[integration] destroying a span nullifies its grades rather than deleting" do
    ...
    grade = ActionGrade.create!(event_valid_attrs(agent_activity: span))

    assert_no_difference -> { ActionGrade.count } do
      span.destroy
    end

    assert_nil grade.reload.agent_activity_id, "the grade survives the span, orphaned"
  end
  ...
  # dereferencing agent_action unconditionally — a span grade has no action.

  test "[unit] insight_source returns the action for an action grade" do
    a = action
    grade = ActionGrade.new(valid_attrs(agent_action: a))
  ...
  test "[unit] insight_source returns the span for a span grade (never nil-derefs an action)" do
    span = event
    grade = ActionGrade.new(event_valid_attrs(agent_activity: span))
    assert_nil grade.agent_action, "a span grade has no action target"
  ...
  test "[unit] insight_label reads a span by category and an action by event slug" do
    span_grade = ActionGrade.new(event_valid_attrs(agent_activity: event(category: "Verify")))
    assert_equal "Verify", span_grade.insight_label

    a = action(event_slug: "Add the auth gate")
    action_grade = ActionGrade.new(valid_attrs(agent_action: a))
  ...
  def banked_grade(slug:, **overrides)
    g = ActionGrade.create!(valid_attrs(agent_action: action(session_id: "feed-#{slug.object_id}"),
  ...
  test "[unit] to_insight is the compact feed shape and drops nil fields" do
    ...
    grade = ActionGrade.create!(valid_attrs(agent_action: a, slug: "flag the gap first",
  ...
  test "[unit] to_grade_json is the recorded-grade echo shape" do
    ...
    assert_equal span.id, json["agent_activity_id"]
  end
end
