...
class FastCertSubjectTest < Minitest::Test
  ...
  def test_no_config_source_in_this_repo_maps_over_the_cap
    ...
    # 23 → 24 on 2026-09-09 (prepare-skips-engine-own-lock) for the SAME reason a
    # third time: test/lib/release_producer_lock_bump_test.rb is a new file rather
    # than an append to the frozen hotspot test/lib/release_cli_test.rb (7669, at
    # its ceiling), and its header cites this config to say so. That the same cause
