...
class ReleaseCliTest < Minitest::Test
  ...
  # --- local presence: what a sweep/ship publishes about THIS MACHINE ----------
  #
  # Slice 4 of docs/agents/system/agent-presence.md. The board `assembler` claim
  # answers "is a release live" REMOTELY; these assert the LOCAL twin, which is the
  # only fact that decides whether the agent at the next desk may launch a suite.
  # Measured cost of its absence: a 45-minute full-suite run SIGTERMed at its 2700s
  # ceiling, 11% complete, killed by a sweep no status command reported.
  #
  # OBSERVED FROM INSIDE THE WORK, not asserted about the source. Each case stubs
  # `sh` to READ THE CLAIM OFF DISK at the moment the wrapped command runs, so the
  # test can only pass if the transition is actually in force while the work
  # happens — a grep of bin/release.rb would pass against a transition that never
  # fires, and against one that never restores.
  ...
  def test_run_test_scope_publishes_the_suite_weight_for_exactly_its_duration
    Dir.mktmpdir do |root|
      ...
      assert_match(/INSIDE=.*"weight":"suite"/, out,
                   ...
                   "run_test_scope is the choke point every conductor gate passes through")
      assert_match(/AFTER=.*"weight":"light"/, out,
                   ...
                   "stale phase, bounded by its lifetime, never a live writer's bookkeeping")
    end
  end
  ...
  def test_a_release_runs_normally_when_presence_is_disarmed
    setup = SCOPE_EMIT_STUB + <<~RUBY
