# `token-session.md` before the capability cut — archived 2026-09-25

Frozen record. On 2026-09-25 (epic devops-v3, piece 5b) `docs/agents/modules/token-session.md` was cut to a
...
the cut (305 lines), so every passage the live page no longer carries is here
verbatim. Relative links below resolve from the file's original folder, not this one.

Do not edit this file to keep it current. The live rules are in `docs/agents/modules/token-session.md`.
