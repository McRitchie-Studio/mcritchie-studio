...
4. If the work is a feature, bug, QA, release, cleanup, or active-doc change,
   ...
   expected checks, and `devops["worktree_slug"]`. The production task slug is
   immutable and generated by the app; use `devops["worktree_slug"]` for the
   human-readable feature handle. Move the task to `in_progress` once the agent
   starts work.
...
9. Commit and push the feature branch when the work is coherent, then run
   ...
   acceptance criteria, then move it to `pr_review`. Handoffs should include
   the task URL before the PR URL.
...
run bin/agent-worktree finish, update the task with branch/PR/local
URL/checks, and move it to pr_review for Avi QA. Lead the handoff with the task
URL. Do not merge or deploy unless I explicitly assigned that lane.
```
