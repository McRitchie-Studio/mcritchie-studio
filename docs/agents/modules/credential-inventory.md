...
| `heroku.studio.applications` | `studio-applications` | Heroku **CI lane** OAuth authorization (`d976c7b8…`), scope `identity,read-protected,write-protected` (same matrix as the agents lane; in the item notes). Runtime home is the Actions `HEROKU_API_KEY` secret at cutover; staged as `HEROKU_STUDIO_APPLICATIONS_API_KEY` in `~/.zprofile.admin`. Replaced `mcritchie-studio.github-actions-heroku` (global-scope lane `1f468f88…`, revoked 2026-09-02). Rotate with `bin/rotate-heroku-ci-key` (`--dry-run` first); it keeps `authorization-id` current. | GitHub Actions deploys |
...
| `github.mcritchie-admin` | `studio-agents-admin` | GitHub App for the **ship/admin** lane: Contents + Actions + Checks read + Secrets + Environments — **no pull-request scope by design** (it cannot open or merge PRs). Renamed from `mcritchie-deployer` on 2026-09-26 (same app id and key); the old item `github.mcritchie-deployer` stays as a digest-identical copy and still routes to this vault until it is retired. Fields: `app-id` = **`4431542`** (recorded here on purpose — see **GitHub App IDs** below), `client-id`; the key is the FILE attachment, here named `privatekeypem` (no `.pem` suffix; the helper picks it). | `production-deploy` / `bin/release ship` sessions, via `export GH_APP_ITEM=github.mcritchie-admin` — in a shell that has run `source ~/.zprofile.admin` first, because only the admin token can read this vault. |
...
| deployer (ship) | `github.mcritchie-admin` | `studio-agents-admin` | **`4431542`** |
...
is a self-read; the same call for `mcritchie-admin` from the agent's token is
