...
# RESUBMISSION demo pair (feature: rework-resubmission-looks-like-build). A
# `--kind rework` block leaves the task on `building`, so a bounced task and a
# never-reviewed one used to render identically. These two are the states a reader
# must be able to tell apart, and they differ ONLY in whether the PR head moved
# after the send-back — which is the honest signal, since unresolved_feedback holds
# prose and is cleared by ceremony rather than by the work landing.
#
# run_started_at on every run below is OLDER than the /deployments Actions panel run
# (20.minutes.ago) for the same reason the CI-progress fixtures are: latest_per_workflow
# is GLOBAL, one row per workflow_name, so a newer "CI" row here would hijack that panel.
resub_unmoved = Task.create!(
  title: "Resubmission unmoved demo",
  slug: "e2e-resubmission-unmoved",
  description: "Sent back for rework; the PR head has NOT moved since the bounce.",
  stage: "building",
  priority: 1,
  agent_slug: "carl",
  metadata: { "devops" => {
    "kind" => "bug", "repositories" => ["mcritchie-studio"],
    "branch" => "feat/e2e-resubmission-unmoved",
    "pr_url" => "https://github.com/McRitchie-Studio/mcritchie-studio/pull/513"
  } }
)
GithubWorkflowRun.create!(
  repo: "McRitchie-Studio/mcritchie-studio", run_id: 5_000_201, status: "completed", conclusion: "success",
  workflow_name: "CI", head_sha: "029a945b1c2d3e4f5061728394a5b6c7d8e9f001",
  head_branch: "feat/e2e-resubmission-unmoved", run_started_at: 90.minutes.ago
)
Activity.create!(
  task_slug: resub_unmoved.slug, activity_type: "qa_feedback", agent_slug: "avi",
  description: "The sibling's both-copies claims survive this change unaltered, so the " \
               "counts it asserts are still the old ones. Rebind them to the new pair and resubmit.",
  metadata: { "kind" => "rework", "summary" => "Sibling both-copies claims survive" },
  created_at: 70.minutes.ago
)

resub_addressed = Task.create!(
  title: "Resubmission addressed demo",
  slug: "e2e-resubmission-addressed",
  description: "Sent back for rework; the PR head MOVED after the bounce.",
  stage: "building",
  priority: 1,
  agent_slug: "carl",
  metadata: { "devops" => {
    "kind" => "bug", "repositories" => ["mcritchie-studio"],
    "branch" => "feat/e2e-resubmission-addressed",
    "pr_url" => "https://github.com/McRitchie-Studio/mcritchie-studio/pull/514"
  } }
)
GithubWorkflowRun.create!(
  repo: "McRitchie-Studio/mcritchie-studio", run_id: 5_000_202, status: "completed", conclusion: "success",
  workflow_name: "CI", head_sha: "029a945b1c2d3e4f5061728394a5b6c7d8e9f002",
  head_branch: "feat/e2e-resubmission-addressed", run_started_at: 90.minutes.ago
)
Activity.create!(
  task_slug: resub_addressed.slug, activity_type: "qa_feedback", agent_slug: "avi",
  description: "Same finding as its sibling: rebind the assertions to the new pair and resubmit.",
  metadata: { "kind" => "rework", "summary" => "Sibling both-copies claims survive" },
  created_at: 70.minutes.ago
)
GithubWorkflowRun.create!(
  repo: "McRitchie-Studio/mcritchie-studio", run_id: 5_000_203, status: "completed", conclusion: "success",
  workflow_name: "CI", head_sha: "7b3f0c8e9d1a2b3c4d5e6f708192a3b4c5d6e7f8",
  head_branch: "feat/e2e-resubmission-addressed", run_started_at: 50.minutes.ago
)
