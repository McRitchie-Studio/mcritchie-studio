...
# [unit] No JavaScript comment inside a hub view's script block may carry an
# ERB sequence.
#
...
# and the anti-vacuous floors re-measured for hub — the walker, the leak
...
# scan for.
#
# MEASURED BEFORE SHIPPING, on origin/accepted on 2026-09-01: 212 view files
# holding 28 script blocks, 463 JavaScript comments and 31_858 bytes of comment
# body that no guard in this repo read. The scan returned ZERO findings, so this
# lands green with NO allowlist — the only way it is worth landing, since an
# allowlisted guard is how a rule stops guarding. The sibling consumer,
# turf-monster, returned exactly one on the same day (an escaped tag in its
# layout's modal-host comment), which is why both repos were enrolled together.
#
...
class ScriptCommentLeakTest < ActiveSupport::TestCase
  ...
  # Anti-vacuous floors. Measured on origin/accepted on 2026-09-01: 28 script
  # blocks, 463 comments and 31_858 bytes of comment body over 212 view files.
  ...
  MIN_SCRIPT_BLOCKS = 16
  MIN_COMMENTS = 275
  MIN_COMMENT_BYTES = 19_000
  ...
  test "no script comment in a hub view carries an ERB sequence" do
    found = leaking_script_comments

    assert_empty found,
                 "these JavaScript comments live inside a script block in a hub view and " \
