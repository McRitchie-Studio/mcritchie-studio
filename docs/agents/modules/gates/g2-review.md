...
The four gates in order: [G1 Cert](g1-cert.md) → **G2 Review** (this doc) →
...
- **The supervisor** (`bin/pr-review` — Avi's `pr-review` /
  ...
  code himself.
- **The primary reviewer** contributes the gate-zero SOP entry
  (`bin/dor-check <slug> --gate-role review`).
...
2. **Gate-zero** — the primary's prompt instructs it to run

   ```bash
   bin/dor-check <task-slug> --gate-role review
   ```

   which appends its verdict as a `dor-check` SOP on the **open G2a attempt**
   (never closing the builder's G1 Cert — that is exactly what the
   `--gate-role review` flag exists for).
...
- [`g1-cert.md`](g1-cert.md) — the builder gate whose dor-check this gate's
  gate-zero re-runs with `--gate-role review`.
